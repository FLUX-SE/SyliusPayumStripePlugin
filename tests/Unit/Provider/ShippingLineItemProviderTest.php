...
final class ShippingLineItemProviderTest extends TestCase
{
    ...
    public function testGetLineItem(): void
    {
        ...
        $this->shippingLineItemNameProviderMock->expects($this->once())->method('getItemName')->with($orderMock)->willReturn('My shipping method');
        $orderMock->expects($this->once())->method('getShippingTotal')->willReturn(1000);
    ...
    public function testGetLineItemWhenThereIsNoShipping(): void
    {
        /** @var OrderInterface&MockObject $orderMock */
        $orderMock = $this->createMock(OrderInterface::class);
        $orderMock->expects($this->once())->method('getShippingTotal')->willReturn(0);
