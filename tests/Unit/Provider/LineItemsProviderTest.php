...
final class LineItemsProviderTest extends TestCase
{
    ...
    public function testGetLineItems(): void
    {
        ...
        $this->lineItemProviderMock->expects($this->once())->method('getLineItem')->with($orderItemMock)->willReturn($lineItem);
        $this->shippingLineItemProviderMock->expects($this->once())->method('getLineItem')->with($orderMock)->willReturn(null);
        $this->assertSame([
            $lineItem,
